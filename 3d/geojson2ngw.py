...
def process():
    ...
    if create:
        ...
        grpid = PARENT
        grpref = dict(id=grpid)    
    print('created group'+str(grpref))
        ...
        for filename in fnames:
            ...
            if ('.geojson' in repr(filename)) or ('.GEOJSON' in repr(filename)): #apply lower to filename as it fails for cyrillic filename
                ...
                vectlyr = post(courl(), json=dict(
                    resource=dict(cls='vector_layer', parent=grpref, display_name=os.path.splitext(filename)[0], keyname=keyname),
    ...
    def search_layer_by_keyname_group(keyname,group):
        print('search for layer with keyname={keyname} in group={group}'.format(keyname=keyname,group=group))
        serach_results = get(courl()+'search/?keyname='+keyname)
        for result in serach_results:
            print(result)
            if (result['resource']['parent']['id']==group):
    ...
    keyname='polygonz'
    serach_results = get(courl()+'search/?keyname='+keyname)
    layer_id = search_layer_by_keyname_group(keyname='polygonz',group=grpref['id'])
    print('layer_id='+str(layer_id))
  
    style = post(courl(), json=dict(
        resource=dict(cls='style_3d', parent=dict(id=layer_id), display_name=keyname+'3d2'),
        style_3d=dict(
    style_type= "GEOJSON", 

    gj_height= 320.0, 
    gj_height_field= None, 
    gj_height_type= "VALUE"

    )
    ))
    print('created '+courl() + '/' + str(style))
    styles43dmap['polygonz']=style
